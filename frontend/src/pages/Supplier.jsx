...
const Supplier = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.code || !form.name) {
      Swal.fire('Peringatan', 'Kode dan nama supplier harus diisi', 'warning');
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="page active">
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={openAdd}>
          <i className="fas fa-plus"></i>Tambah Supplier
        </button>
      </div>
      ...
      <div className={`modal ${showModal ? 'active' : ''}`} onClick={e => e.target === e.currentTarget && closeModal()}>
        <div className="modal-content">
          <div className="modal-header">
            <span className="modal-title">{editId ? 'Edit Supplier' : 'Tambah Supplier'}</span>
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              <div className="form-group">
                ...
                <input className="form-input" name="code" value={form.code} onChange={handleChange} placeholder="Contoh: SUP-05" required />
              </div>
              <div className="form-group">
                <label className="form-label">Nama Supplier <span style={{ color: 'red' }}>*</span></label>
                <input className="form-input" name="name" value={form.name} onChange={handleChange} placeholder="Nama supplier" required />
              </div>
              <div className="form-group">
